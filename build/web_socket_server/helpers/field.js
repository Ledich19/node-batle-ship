...
export const createPoints = (field, x, y, rules) => {
...
export const checkIsAliveShip = (field, x, y) => {
    const startX = x;
    const startY = y;
    const visited = [];
    const recursiveCheck = (field, x, y) => {
        var _a;
        if (typeof x !== 'number' || typeof y !== 'number' || ((_a = visited[y]) === null || _a === void 0 ? void 0 : _a[x])) {
            return false;
        }
        visited[y] = visited[y] || [];
        visited[y][x] = true;
...
export const findDAmageShip = (field, x, y, points) => {
...
export const createKilledShip = (field, x, y, currentPlayer) => {
    const points = [];
    let point = {
        x: x,
        y: y,
    ...
    for (let i = 0; i < iterations; i++) {
        const pointX = points[i].position.x;
        const pointY = points[i].position.y;
...
export const getRandomCeil = (field) => {
    const possibilityOfShot = [];
    field.forEach((row, i) => {
        row.forEach((col, j) => {
...
export const checkShoutResult = (field, position) => {
    const point = field[position.y][position.x] === SHIP
        ? DAMAGE
...
function determineOrientation(cell1, cell2) {
    if (cell1.x === cell2.x) {
        return true;
    }
    else {
...
function findMinMaxProperty(arr, property) {
    const values = arr.map((item) => item[property]);
    const max = Math.max(...values);
    const min = Math.min(...values);
    return { max, min };
}
export const createShootOption = (field, x, y) => {
